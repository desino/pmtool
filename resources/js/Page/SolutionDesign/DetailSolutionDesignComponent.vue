<template>
    ...
    <div class="app-content row  mh-100">
                        ...
                        <div class="d-flex align-items-center section-container">
                            <div class="fw-bold fs-5">
                                <i :aria-controls="'collapse_' + section.id"
                                   :aria-expanded="!collapsedSections[section.id]" :class="['bi', collapsedSections[section.id] ? 'bi-caret-right-fill' : 'bi-caret-down-fill']"
                                   :data-bs-target="'#collapse_' + section.id"
                                   data-bs-toggle="collapse"
                                   @click="collapsedSections[section.id] = !collapsedSections[section.id]"></i>
                                <a href="javascript:void(0)" @click="scrollToSection(section.id)" class="text-decoration-none text-dark">{{ section.display_name }}</a>
                            </div>
                        </div>
                    </div>
                    <div v-if="section.functionalities" :id="'collapse_' + section.id"
                         :class="{'show': !collapsedSections[section.id]}"
                        ...
                        <div v-for="functionality in section.functionalities" :key="functionality.id">
                            <div :class="['list-group-item d-flex list-group-item-action', { 'bg-desino text-light': isSelected(functionality.id) }]"
                                 ...
                                 role="button" @click="scrollToFunctionality(section.id, functionality.id)">
        ...
        <div class="col-md-8 border-start border-bottom border-top">
            <section class="p-2 border-bottom section_detail" v-for="section in sectionsWithFunctionalities" :key="section.id" :id="'section_' + section.id">
                <h5 class="mb-2 text-primary"># {{ section.display_name }}</h5>
                <div v-if="section.functionalities.length > 0" class="px-4" v-for="functionality in section.functionalities" :key="functionality.id" :id="'functionality_' + functionality.id">
